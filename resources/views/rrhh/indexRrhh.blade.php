    ...
    <div class="min-h-screen bg-slate-100 py-16 px-6 flex flex-col items-center">
        <div class="w-full max-w-5xl">
            ...
            <div class="bg-white rounded-3xl shadow-xl p-10 border-2 border-orange-200">
                ...
                <form action="{{ route('searchRrh.rrhh') }}" method="POST" class="flex justify-between items-center mb-6 border-b-2">
                    @csrf
                    <input type="text" name="search" class="w-11/12 border-0 bg-transparent px-3 py-2 outline-none ring-0 focus:ring-0 transition-all" placeholder="Busca RH pendent per l'estat o el context...">
                ...
                <div class="space-y-6">
                    @forelse($rrhhs as $rrhh)
                        <div class="divMaintCard transition-all duration-300">
                                <li x-data="{ open: false }" class=" border border-orange-200 rounded-xl overflow-hidden m-3">
                                    ...
                                    <div x-show="open" x-collapse class="bottomDiv bg-white px-6 pb-4 border-t border-orange-100">
                                        <div class="flex mb-4 w-full bg-white border-b border-l border-r border-orange-100 rounded-b-lg overflow-hidden">
                                            ...
                                            <a href="{{ route('rrhh.delete',$rrhh->id)}}" class="w-full text-center px-4 py-2 text-red-600 hover:bg-orange-50 transition ">
                                                Eliminar
                                            </a>
                                        </div>
                                ...
                                <!-- Tracking -->    
                                <div class="trackingDiv fixed inset-0 backdrop-blur-sm items-center justify-center hidden">
                                    <div class="bg-white/95 rounded-3xl p-6 w-full max-w-lg shadow-xl border border-orange-200">
                                ...
                                <div class="editForm fixed inset-0 backdrop-blur-sm bg-black/40 items-center justify-center z-50 px-4 hidden">
                                    <div class="bg-white/95 rounded-3xl p-6 w-full max-w-lg shadow-xl border border-orange-200">
                                        ...
                                        <h1 class="text-orange-500 font-bold text-2xl text-center mb-6 ">Editar RRHH</h1>
                                        <div id="rrhhForm">
                                            <form action="{{route('rrhh.update',$rrhh)}}" method="POST" class="space-y-4" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                                <div>
                                                    <label class="block text-sm text-orange-600 mb-1 font-medium">Context</label>
                                                    <input type="text" name="context" value="{{ $rrhh->context }}"
                                                        class="w-full border border-orange-200 bg-orange-50 focus:border-orange-400 focus:ring-orange-400 px-3 py-2 rounded-xl outline-none transition"
                                                    >
                                                </div>
                                                <div>
                                                    ...
                                                    <textarea name="description" 
                                                        class="w-full border border-orange-200 bg-orange-50 focus:border-orange-400 focus:ring-orange-400 px-3 py-2 rounded-xl outline-none transition"
                                                    >{{ $rrhh->description }}"</textarea>
                                                </div>
                                                <!--
                                                <div>
                                                    <label class="block text-sm text-orange-600 mb-1 font-medium">Profesional afectat</label>
                                                    <select name="professional_afectat"
                                                        class="w-full border border-orange-200 bg-orange-50 focus:border-orange-400 focus:ring-orange-400 px-3 py-2 rounded-xl outline-none transition"
                                                    >
                                                        <option value="{{ $rrhh->professional_afectat }}" selected> {{ $rrhh->professional_afectat }} </option>
                                                        @foreach($professionals as $professional)
                                                            <option value="{{ $professional->id }}">{{ $professional->name }} {{ $professional->surname1 }} {{ $professional->surname2 }}</option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div>
                                                    <label class="block text-sm text-orange-600 mb-1 font-medium">Profesional derivat</label>
                                                    <select name="professional_derivat"
                                                        class="w-full border border-orange-200 bg-orange-50 focus:border-orange-400 focus:ring-orange-400 px-3 py-2 rounded-xl outline-none transition"
                                                    >
                                                            <option value="{{ $rrhh->professional_derivat }}" selected> {{ $rrhh->professional_derivat }} </option>
                                                        @foreach($professionals as $professional)
                                                            <option value="{{ $professional->id }}">{{ $professional->name }} {{ $professional->surname1 }} {{ $professional->surname2 }}</option>
                                                        @endforeach
                                                    </select>
